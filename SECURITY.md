...
1. **GitHub Security Advisories** (preferred): [Report a vulnerability](https://github.com/nidorx/orqen/security/advisories/new)
...
Orqen is a local-first CLI tool that writes to a local filesystem and SQLite database. The attack surface is intentionally small:
...
- **Out of scope**: issues requiring physical access to the machine where Orqen is installed, or issues that require the attacker to already have access to the user's home directory
