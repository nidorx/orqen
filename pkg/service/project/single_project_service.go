...
func (s *Service) OnStart() error {
	...
	proj.WithInvoker(func(prompt string, itm *project.WorkItem) error {
		...
		return agent.Exec(itm.Name, lane.Name, cwd, prompt, command, []acp.McpServer{
			{
...
func loadProject() *project.Project {
	...
	if err == nil {
		...
		if readErr == nil {
			...
			if answer == "" || strings.EqualFold(answer, "Y") || strings.EqualFold(answer, "YES") {
				...
				if loadErr == nil {
					cli.Printf(messages, "project_loaded", cwd, len(proj.Modules), proj.Agents.Default, proj.Execution.SleepIntervalSeconds)
					return proj
				}
				cli.Printf(messages, "error_loading", loadErr)
			}
		}
	}
	...
	for {
		...
		cli.Printf(messages, "project_loaded", projectDir, len(proj.Modules), proj.Agents.Default, proj.Execution.SleepIntervalSeconds)
		return proj
	}
}
