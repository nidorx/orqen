...
func TestFormatHistoryForPrompt_Basic(t *testing.T) {
	msgs := []ChatMessage{
		{Role: RoleUser, Content: "hello"},
		{Role: RoleAssistant, Content: "hi there"},
	}

	got := FormatHistoryForPrompt(msgs)
	want := "user: hello\nassistant: hi there\n"
	if got != want {
		t.Errorf("got:\n%q\nwant:\n%q", got, want)
	}
...
func TestFormatHistoryForPrompt_SystemSkip(t *testing.T) {
	msgs := []ChatMessage{
		{Role: RoleUser, Content: "hello"},
		{Role: RoleSystem, Content: "system directive"},
		{Role: RoleAssistant, Content: "hi there"},
	}

	got := FormatHistoryForPrompt(msgs)
	if strings.Contains(got, "system directive") {
		t.Errorf("system message should be skipped, got:\n%s", got)
	}
	want := "user: hello\nassistant: hi there\n"
	if got != want {
		t.Errorf("got:\n%q\nwant:\n%q", got, want)
...
func TestFormatHistoryForPrompt_Truncation(t *testing.T) {
	...
	// The truncated content part should be maxMessageFormatLen + truncation suffix
	if len(got) > maxMessageFormatLen+50 {
		t.Errorf("truncated message too long: %d chars", len(got))
	}
}

func TestFormatHistoryForPrompt_TotalLimit(t *testing.T) {
	// Create enough messages to exceed 8000 chars total
	var msgs []ChatMessage
	for i := 0; i < 20; i++ {
		msgs = append(msgs, ChatMessage{
			Role:    RoleUser,
			Content: strings.Repeat("x", 500), // 500 chars * 20 = 10000+ chars
		})
	}

	got := FormatHistoryForPrompt(msgs)
	if len(got) > maxTotalPromptLen+50 {
		t.Errorf("total prompt too long: %d chars (max ~%d)", len(got), maxTotalPromptLen+50)
	}
	if !strings.Contains(got, "...[middle truncated]...") {
		t.Error("expected middle truncation marker")
	}
}
...
func TestSearchConversations(t *testing.T) {
	...
	if results[0].UserID != "user1" {
		t.Errorf("result UserID = %q, want %q", results[0].UserID, "user1")
	}
}
