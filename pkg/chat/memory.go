...
func (s *ChatStore) CreateSession(userID string, ttl time.Duration) (*ChatSession, error) {
	now := time.Now().UTC()
	...
	_, err := s.db.Exec(
		`INSERT INTO chat_sessions (id, user_id, created_at, expires_at, active)
		 VALUES (?, ?, ?, ?, 1)`,
...
func (s *ChatStore) LoadSession(sessionID string) (*ChatSession, error) {
	...
	var active int
	err := s.db.QueryRow(
		`SELECT id, user_id, created_at, expires_at, active
		 FROM chat_sessions WHERE id = ?`,
		sessionID,
	).Scan(&sess.ID, &sess.UserID, &sess.CreatedAt, &sess.ExpiresAt, &active)
...
// given user. Returns nil if none found.
func (s *ChatStore) GetActiveSession(userID string) (*ChatSession, error) {
...
// ExpireOldSessions deactivates all sessions for the given user whose expires_at
// is before now. Returns the count of affected rows.
func (s *ChatStore) ExpireOldSessions(userID string, ttl time.Duration) (int64, error) {
	now := time.Now().UTC().Format(time.RFC3339)
	res, err := s.db.Exec(
...
// ExpireAllSessions deactivates all active sessions for the given user,
// regardless of expiry time. Used when the user explicitly requests a new session.
func (s *ChatStore) ExpireAllSessions(userID string) (int64, error) {
	res, err := s.db.Exec(
		`UPDATE chat_sessions SET active = 0
...
func (s *ChatStore) Search(userID string, query string, limit int) ([]SearchResult, error) {
	if limit <= 0 {
		limit = SearchLimit
	}
	...
	rows, err := s.db.Query(
		`SELECT m.id, m.session_id, cs.user_id, m.role, m.content, fts.rank, m.created_at
		 FROM chat_messages_fts fts
		 JOIN chat_messages m ON m.id = fts.rowid
		 JOIN chat_sessions cs ON cs.id = m.session_id
		 WHERE fts.content MATCH ? AND cs.user_id = ?
		 ORDER BY fts.rank
		 LIMIT ?`,
	...
	for rows.Next() {
		var r SearchResult
		if err := rows.Scan(&r.MessageID, &r.SessionID, &r.UserID, &r.Role, &r.Content, &r.Rank, &r.CreatedAt); err != nil {
