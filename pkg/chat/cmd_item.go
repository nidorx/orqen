...
func handleItem(ctx context.Context, args string, bot *TelegramBot, userID string) (string, error) {
	proj := bot.Project
	if proj == nil {
		return "No project loaded.", nil
	}

	workItemID := strings.TrimSpace(args)
	if workItemID == "" {
	...
	if item == nil {
		return fmt.Sprintf("Workitem %q not found.", workItemID), nil
	}
