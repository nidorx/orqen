...
func newLogger(agent, lane, item string) Logger {
	// [•_•] [claude] [inbox] [create-new-task]
	// [•_•] [qwen] [review] [TASK-0001-add-new-agent] : tool call -
	return Logger(nextColor() + fmt.Sprintf("[%s] \033[90m[%s] [%s] - \033[0m", agent, lane, item))
}
