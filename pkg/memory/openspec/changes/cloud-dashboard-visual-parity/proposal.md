...
6. **Templ source ports** (batch 3): `components.templ`, `layout.templ`, `login.templ` — copy verbatim, adapt import path (`github.com/nidorx/orqen/pkg/memory/cloud/cloudstore`), adapt component signatures to flat-row types. Run `go tool templ generate` — check in `*_templ.go`.
