...
# - Imagem de capa e formatação rica
  ...
  - name: conteudo
    ...
    extra_prompt: |
      ...
      **IDIOMA**: TODO conteúdo deve ser em PORTUGUÊS DO BRASIL (pt-BR).
      Se o artigo for bilíngue, a versão secundária (EN) deve ser tradução fiel,
      não reescrita.

      **SEO DO MEDIUM**: Todo artigo deve ter:
      - Título: impactante, claro, com palavras-chave
      - Subtítulo: complementar, não redundante
      - 5 tags do Medium (mix de amplas e nichadas)
      - URL canônica se o conteúdo foi publicado em outro lugar primeiro

      **TEMPLATES**: Para cada artefato gerado, USE O TEMPLATE CORRESPONDENTE no diretório
      `.orqen/conteudo/prompts/` como base estrutural. Preencha todas as seções do template.
      Os templates são:
      - IDEIA.md        → Análise de ideia com ângulo estratégico
      - PESQUISA.md     → Pesquisa profunda com referências
      - RASCUNHO.md     → Artigo completo pronto para revisão
      - REVISAO.md      → Checklist de revisão e feedback
    ...
    lanes:
      ...
      - name: "inbox"
        ...
        agent_behavior:
          ...
          - "Use o tool workitem_create (orqen MCP Server) para criar um novo item na lane revisar_ideia"
          ...
          - "Finalize a execução"
        critical_rules:
          - "NUNCA avançar se o tema for superficial demais para um artigo longo"
          - "TODO conteúdo em PORTUGUÊS DO BRASIL"
          - "Se a ideia for melhor como post LinkedIn, sugira isso no arquivo e finalize"
        extra_prompt: |
          Um bom artigo do Medium responde a uma pergunta que engenheiros seniores se fazem
      ...
      - name: "pesquisa"
        ...
        agent_behavior:
          - "Leia MED-${SEQUENCE}-IDEIA.md para entender o tema, ângulo e abordagem aprovados"
          - "Pesquise o tema em profundidade: papers, posts de engenharia de empresas reconhecidas, documentação oficial, books"
          ...
          - "Use o tool workitem_move (orqen MCP Server) para mover o item da lane pesquisa para rascunho"
          - "Finalize a execução"
        critical_rules:
      ...
      - name: "rascunho"
        ...
        agent_behavior:
          ...
          - "Use o tool workitem_move (orqen MCP Server) para mover o item da lane rascunho para revisar_conteudo"
          - "Finalize a execução"
        critical_rules:
          - "TODO conteúdo em PORTUGUÊS DO BRASIL"
        ...
        extra_prompt: |
          **REGRAS DE TOM E ESTILO**:
          - ORDEM DIRETA: sujeito + verbo + complemento. Sempre.
          - ZERO travessões. ZERO construções contrastivas. ZERO negação retórica.
          - VOZ ATIVA sempre, salvo impossibilidade gramatical.
          - Primeira pessoa para opinião/experiência: "Eu vi...", "Na minha experiência..."
          - Terceira pessoa para dados: "Um estudo do GitHub Research mostrou..."
          - Sem emojis, sem tabelas decorativas, sem headers estilo marketing.
      ...
      - name: "publicacao"
        ...
        agent_behavior:
          ...
          - "Use o tool workitem_move (orqen MCP Server) para mover o item para publicado"
          - "Finalize a execução"
        critical_rules:
        ...
        extra_prompt: |
          A publicação no Medium envolve mais que clicar 'Publish'. Inclua no checklist:
          - Como formatar no editor do Medium
          - Como adicionar imagem de capa
          - Como configurar SEO (title, subtitle, tags)
...
### IDEIA (`.orqen/conteudo/prompts/IDEIA.md`)
Análise de ideia para artigo longo. Inclui: tema, categoria (Arquitetura, Liderança, IA/ML, Estudo de Caso, etc.), hipótese de valor, ângulos possíveis (3-5), profundidade estimada, público-alvo, conexão com posicionamento, perguntas de esclarecimento, veredito (avançar/rejeitar).

### PESQUISA (`.orqen/conteudo/prompts/PESQUISA.md`)
Pesquisa profunda. Inclui: conceitos centrais, referências acadêmicas/técnicas com links diretos, posts de engenharia de empresas, dados e métricas da indústria, perspectivas divergentes e contra-argumentos, case studies, exemplos de código potenciais, key takeaways.
...
| Regra | Exemplo |
|-------|---------|
| **Mínimo 1500 palavras** | Se menos, o tema é insuficiente para Medium |
| **Sem travessões** (em-dashes) | Use vírgulas ou pontos em vez de `-` ou `–` |
| **Sem construções contrastivas** | Não use "não é X, mas Y" - vá direto ao ponto |
| **Ordem direta** | Sujeito + verbo + complemento |
| **Voz ativa** | Sempre, salvo impossibilidade gramatical |
| **Referências verificáveis** | Nunca inventar referências - incluir links diretos |
| **Código funcional** | Não pseudocódigo - deve ser testável |
| **Público-alvo** | Engenheiros seniores e CTOs |
...
- Escrever **artigos técnicos longos** (1500+ palavras)
- Conteúdo que exige **pesquisa profunda** com referências
- Manter **qualidade editorial** consistente
- **Distribuição cruzada** (Medium + LinkedIn + comunidades)

## Próximos passos

- [Portal Autônomo](exemplos/portal-autonomo.md) - Automação via Telegram
